 ...
 * @final
 */
class ApiRequestPayloadValueResolver implements ValueResolverInterface, EventSubscriberInterface
{
    /**
    ...
    public function onKernelControllerArguments(ControllerArgumentsEvent $event): void
    {
        ...
        foreach ($arguments as $i => $argument) {
            ...
            if ($this->validator) {
                ...
                // Bypass exception for DTO input params validation and add validation errors to request
                if (\count($violations)) {
                    // Throw exception if payload is not DTO
                    if (!str_contains($type, 'App\\DTO\\')) {
                        throw new HttpException($validationFailedCode, implode("\n", array_map(static fn ($e) => $e->getMessage(), iterator_to_array($violations))), new ValidationFailedException($payload, $violations));
                    }

                    $request->attributes->set('_dto_validation_errors', $violations);
                } else {
                    $request->attributes->set('_dto_validation_errors', null);
                }
            } else {
                try {
                    $payload = $this->$payloadMapper($request, $type, $argument);
